...
class User
{
  ...
  public function insertUser($table, $data)
  {
    ...
    // $sql = "INSERT INTO $table ( username, password, role, is_active, ma_gv) VALUES (:username,:password,:role,:is_active,:ma_gv)";
    // $stmt = $this->conn->prepare($sql);
  ...
  public function getTeacherNoUser()
  {
    $sql = "SELECT * FROM giang_vien WHERE ma_gv NOT IN (SELECT ma_gv FROM tai_khoan)";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
  }
